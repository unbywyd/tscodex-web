...
//
// CORS открыт всем: сюда ходят чужие клиенты из чужих браузеров, и заранее
// перечислить их источники невозможно. Ограничение списком защитило бы ровно
// ничего — сервер и так хранит шифротекст, а доступ даёт знание
// идентификатора, а не адрес страницы, с которой пришёл запрос.
app.use('/api/v1/rooms/*', cors({ origin: '*' }))
